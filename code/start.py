...
import json
#XXX working on tensorboard; tensorboard sucks
		...
		for epoch in range(p['epochs']):
			for i, (sample, label) in enumerate(train_loader):
				...
				if (i+1) % ip_ratio == 0:
					...
					error = get_error(model_output, label)
					total_error += error
...
def test(p, model, criterion, test_loader):
	...
	loss_plot(p, total_loss_lst, avg_err_lst)
	# Need to return a scalar for the objective function
	return total_loss
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ TEST ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

def get_error(model_output, target):
...
#NOTE NEVER TRY TO UPLOAD THE tb.whatever OBJ TO GITHUB OR RSYNC IT -- TOO BIG
...
#def run_model(p, samples_dict):
def run_model(p):
	samples_dict = p['samples_dict']

	if p['load_model'] == True:
		#sd is an individual cluster sample_dict
		for key, sd in samples_dict.items():
			train_loader, test_loader = get_data(p, sd)
			# Loads the state_dict then feeds into model
			cluster_filename = str(str(key) + p['model_filename'])
			model.load_state_dict(torch.load(cluster_filename))
			total_loss = test(p, model, criterion, test_loader)
		...
		for key, sd in samples_dict.items():
			...
			total_loss = test(p, model, criterion, test_loader)
			cluster_filename = str(str(key) + p['model_filename'])
			torch.save(model.state_dict(), cluster_filename)

	#XXX may error since I didn't initialize to 0
	#NOTE necessary return to qualify as an optimization function
	return total_loss
...
if __name__ == "__main__":
	...
	if parameters.bayesian_optimization == False:
		p['samples_dict'] = samples_dict
		#run_model(p, samples_dict)
		run_model(p)
		
	elif parameters.bayesian_optimization == True:
		new_p['samples_dict'] = samples_dict
		layers_p['samples_dict'] = samples_dict
		algo = tpe.suggest
		# optimizing number of hidden layers trials object
		trials1 = Trials()
		best_layers = fmin(
			run_model, layers_p, 
			algo=algo, max_evals=1, trials=trials1)
		log.info('OPTIMIZED HIDDEN LAYERS: {}'.format(best_layers))
		#XXX not sure this is how we access the returned obj from fmin()
		new_p['n_hidden_layers'] = best_layers['n_hidden_layers']
		parameters.make_hiddens(hp_p, new_p)
		# optimizing the rest of the hyperparams trials object
		trials2 = Trials()
		best_hp = fmin(
			run_model, new_p, 
			algo=algo, max_evals=1, trials=trials2)
		log.info('OPTIMIZED HYPERPARAMS: {}'.format(best_hp))

	#XXX KMEANS TODO
	# *
	# Adjust for iterating and training, iterating and testing, and saving
	# w different names (as done with processed clusters in dp)
	# *
	# Adjust loading to iteratively load if kmeans == True?

	# Adjust plotting (HOPEFULLY USING SEABORN) to display each model's
	# graph next to anothers, and also save them in a new dir for models

	# Make model, criterion and optimzer parameters and adjust 
	# EVERYTHING accordingly
	# *
	# Fix inputting nans to zipped volumes etc...
	#XXX

	#XXX BAYESIAN TODO
	
	# Write logic to control whether start.py runs normally or with optimization
	# Also make optimization function that combines the creation of the model
	# and the training and the testing and still returns loss
	#NOTE make sure to pass in new_p and not p for this
	
	# Write parameter that controls that
	
	# Put the pieces together... fmin, objective func, etc...

	# Make total_layers adjustable by hp
		
	# Test that Crypto_Net can construct w variable amount of hiddens

	# Write a test for running the adjusted start.py with Bayesian!

	# Make fake_p have much more restricted params so that it uses less CPU
