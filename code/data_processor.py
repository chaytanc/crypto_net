...
# It should go without saying, but all calls to libraries
...
# parameters is a dict object stored locally
...
class Data_Processor():
	...
	def get_segmented_data(self, df):
		''' 
		Args:
			df: a pandas dataframe with company row numbers in the 0th col.
	...
	def delete_empty_volumes(self, volumes):
		...
		Very similar to delete_empty_labels, but whatever I just made another
	...
	def calculate_k(self, p, possible_clusters, wcss):
		...
		#XXX seaborn not working so good
#		d = {'num_clusters' : possible_clusters, 'sum_of_squared_dist' : wcss}
#		data = pd.DataFrame(d)
#		#long_form = pd.melt(d)
#		splot = sns.relplot(
#			x='num_clusters',
...
#			)
#		splot.fig.suptitle(
#			'K-Means Clustering Sum Squared Error Over n Clusters')
#		plt.show()
	...
	def get_slopes(self, possible_clusters, wcss):
		...
		for i, (x, y) in enumerate(zip(possible_clusters, wcss)):
			...
			# Can't calculate the slope of the last point
			if i != (len(wcss) - 1):
	...
	def average_nans(self, lst):
		...
		''' Input must be 2d lst -- not jagged! This function loops through
	...
	def find_nans(self, lst):
		''' Used to test if a two dimensional list has nans '''
	...
	def main(
		...
		'''
		Returns: samples_dict in the format of a dictionary of dictionar(y/ies)
		in which each dict contains train_samples volumes & test_samples vols
		and their labels.

		Ex of a 1 cluster samples_dict, or a p['kmeans'] == False: 

		{'0' : {'train_samples' : tensor([0, 1, 2, 3]), 
		'train_labels' : tensor([0, 2]),
		'test_samples' : tensor([6, 7]), 
		'train_labels' : tensor([9])}
		}
		'''

		samples_dict = {}
		if p['load_processing'] == True:
			indiv_samples_dict = self.load_train_test_samples(p['obj_filename'])
			samples_dict['0'] = indiv_samples_dict
		else:
			df = self.get_df()
			...
			newer_volumes = self.average_nans(new_volumes)
			if p['kmeans'] == True:
				zipped_volumes = self.get_zipped_volumes_and_devs(newer_volumes)
				vol_clusters = self.k_cluster(p, zipped_volumes, newer_volumes)
				for i, cluster in enumerate(vol_clusters):
					p['obj_filename'] = 'processed_cluster' + str(i) + '.pkl'
					indiv_samples_dict = self.post_volumes_processing(
						p, cluster)
					samples_dict[str(i)] = indiv_samples_dict
					self.save_train_test_samples(
						samples_dict, p['obj_filename'])
			# If load_processing is False and kmeans is False
			else:
				indiv_samples_dict = self.post_volumes_processing(
					p, newer_volumes)
				samples_dict['0'] = indiv_samples_dict
